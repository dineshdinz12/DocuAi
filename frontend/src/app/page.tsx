...
import { FileText, Trash2, ExternalLink, LogOut, LogIn, Plus, MessageSquare, Pencil, Check } from "lucide-react";
      ...
      <main className="flex-1 flex flex-col h-full bg-white relative">
        ...
        <header className="md:hidden flex items-center justify-between p-3.5 border-b border-slate-200 bg-white sticky top-0 z-10">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handleNewChat}
              className="p-1.5 border border-slate-200 rounded-lg text-slate-700 font-medium text-xs flex items-center gap-1"
            >
            ...
            {!user && (
              <button
                onClick={() => setIsAuthModalOpen(true)}
                className="py-1 px-3 bg-slate-900 text-white font-semibold text-xs rounded-lg shadow-sm"
              >
                Sign In
              </button>
            )}
